...
#else
/* Linux/GNU wants this macro, otherwise stdint.h and limits.h are mostly
 * useless */
...
/* rand only returns 15 bits, so we xor 3 calls together to get the full result
 * (13 bits overflow, but that is okay) */
// shifting signed values might overflow and be undefined
#define URAND31() (((unsigned)rand() << 30) ^ ((unsigned)rand() << 15) ^ rand())
// choose to produce a positive or a negative number. Note: conditional only
// evaluates one URAND31
#define RAND32() ((int)(rand() & 1 ? URAND31() : -URAND31() - 1))

/* rand only returns 15 bits, so we xor 5 calls together to get the full result
 * (11 bits overflow, but that is okay) */
// shifting signed values might overflow and be undefined
#define URAND63()                                                              \
  (((uint64_t)rand() << 60) ^ ((uint64_t)rand() << 45) ^                       \
   ((uint64_t)rand() << 30) ^ ((uint64_t)rand() << 15) ^ rand())
// choose to produce a positive or a negative number. Note: conditional only
// evaluates one URAND63
